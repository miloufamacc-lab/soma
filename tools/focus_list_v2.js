...
function setupSheet() {
  ...
  var headers = ["Ticker", "Company", "Price", "GF Value", "Prem/Disc", "GF Score", "Altman Z", "Piotroski F", "Momentum", "Sales Growth", "52w High", "% from High", "Signal", "Updated"];
...
function refreshAllData() {
  ...
  for (var i = 0; i < TICKERS.length; i++) {
    ...
    try {
      ...
      var rowData = [ticker, companyName, price, gfValue, premDisc, gfScore, altmanZ, piotroski, momentum, salesGrowth, high52, pctFromHigh, signal, new Date()];
...
function sendDailyEmail() {
  ...
  data.sort(function(a, b) {
    ...
    return b[12] - a[12]; // Then by signal
  });
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var salesGrowth = data[i][9];
    var signal = data[i][12];
    
    var premDiscColor = premDisc < 0 ? "#22c55e" : "#ef4444";
    var premDiscText = premDisc < 0 ? Math.abs(premDisc * 100).toFixed(0) + "% discount" : (premDisc * 100).toFixed(0) + "% premium";
    ...
    html += `
    <div style="background: ${cardBg}; ${borderLeft} padding: 12px; margin-bottom: 8px; border-radius: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
        <span style="font-weight: bold; font-size: 16px;">${ticker}</span>
        <span style="font-size: 16px; font-weight: 600;">$${price.toFixed(2)}</span>
      </div>
      
      <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 6px;">
        <span style="color: #666;">GF Value: $${gfValue.toFixed(0)}</span>
        <span style="color: ${premDiscColor}; font-weight: 500;">${premDiscText}</span>
      </div>
      
      <div style="display: flex; gap: 8px; flex-wrap: wrap;">
        <span style="background: #f1f5f9; padding: 3px 8px; border-radius: 4px; font-size: 11px;">
          📈 Growth: ${salesGrowth.toFixed(0)}%
        </span>
        <span style="background: #f1f5f9; padding: 3px 8px; border-radius: 4px; font-size: 11px; color: ${momentumColor};">
          ⚡ Mom: ${momentum}/10
        </span>
        <span style="background: #f1f5f9; padding: 3px 8px; border-radius: 4px; font-size: 11px; color: ${piotColor};">
          💪 Piot: ${piotroski}/9
        </span>
      </div>
    </div>
